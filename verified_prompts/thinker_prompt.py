...
thinker_system_prompt = """
 Please respond with the very best of your abilities. Your task is to generate one distinct possible next thinking step in a problem-solving process. Given a Question, a Chain of Thoughts, and a Solution reference, you will:
    
    - Analyze and understand the question.
    - Consider the given reference solution.
    - Propose only one possible next thinking step that builds off the Chain of Thoughts and aligns with the final solution.
    - Ensure the step is concise, short, and simple.
    - Do not directly state the final answer, only provide an intermediate step.
    ...
    Respond only with a single python string containing the next thinking step.
"""
